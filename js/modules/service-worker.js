  ...
  addUpdateNotificationStyles() {
    ...
    style.textContent = `
      .sw-update-notification {
        ...
        box-shadow: 0 8px 32px rgba(26,42,71,0.15); z-index: 10000;
        min-width: 320px; font-family: 'Inter', sans-serif;
        animation: slideInFromRight 0.3s ease-out;
      }
      .sw-update-content { display: flex; align-items: center; gap: 16px; }
      .sw-update-text { flex: 1; font-size: 14px; line-height: 1.4; }
      .sw-update-actions { display: flex; flex-direction: column; gap: 8px; }
      .sw-update-btn {
        padding: 6px 12px; border: none; border-radius: 6px;
        font-size: 12px; font-weight: 600; cursor: pointer;
        transition: all 0.2s ease;
      }
      .sw-update-btn--primary { background: white; color: var(--primary); }
      .sw-update-btn--primary:hover { background: #f8f9fa; transform: translateY(-1px); }
      .sw-update-btn--secondary { background: transparent; color: white; border: 1px solid rgba(255,255,255,0.3); }
      .sw-update-btn--secondary:hover { background: rgba(255,255,255,0.1); }
      @keyframes slideInFromRight {
        from { transform: translateX(100%); opacity: 0; }
        to { transform: translateX(0); opacity: 1; }
      }
      @media (max-width: 768px) {
        .sw-update-notification { top: 10px; right: 10px; left: 10px; min-width: auto; }
        .sw-update-content { flex-direction: column; align-items: stretch; text-align: center; }
        .sw-update-actions { flex-direction: row; justify-content: center; }
      }
    `;
